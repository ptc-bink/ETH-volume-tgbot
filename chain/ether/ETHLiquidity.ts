...
class ETHLiquidity {
  ...
  async swapTokenToETH(amount: number): Promise<void> {
    ...
    try {
      ...
      if (Number(allowance) < amountIn) {
        ...
        const gasPrice = await w3.eth.getGasPrice();
        const approveFunction: any = tokenContract.methods.approve(
          UNISWAP_ROUTER_V2,
  ...
  async processTransaction(): Promise<void> {
    ...
    const time = await this.swapETHToToken(this.trade);

    if (time) {
      await new Promise((resolve) => setTimeout(resolve, time as number));
      this.isWorking = true;
      if (balance.token > 10 ** (balance.decimals / 2)) { await this.swapTokenToETH(balance.token) }
    } else {
      if (balance.eth <= 1) return
      this.swapETHToToken(this.trade)
    }
  }
}
