...
import { get_router_v3_abi, get_router_abi, get_erc20_abi } from '../../utils/utils'; // import utilities
import { MEV_BLOCK_RPC_ENDPOINT, WETH_ADDRESS, UNISWAP_ROUTER_V3, UNISWAP_ROUTER_V2 } from '../../utils/constant'; // import constants
...
class ETHLiquidity {
  ...
  constructor(userId: string, dexId: string, tokenAddress: string, walletAddress: string, secret_key: string, txn: number, speed: string, tradeAmount: number) {
    this.id = userId;
  ...
  async swapETHToToken(amount: number): Promise<number | boolean> {
    ...
    try {
      ...
      const expectedAmountOut = await routerContract.methods.getAmountsOut(
        amount_in_wei,
        [w3.utils.toChecksumAddress(WETH_ADDRESS), w3.utils.toChecksumAddress(this.token_addr)]
      ).call();

      let amount_out_min = expectedAmountOut[expectedAmountOut.length - 1] * (1 - slippage);
      amount_out_min = w3.utils.toWei(amount_out_min.toString(), 'wei');
      if (parseFloat(amount_out_min) <= 0) {
        return false;
      }
      ...
      const tx = await swapFunction.buildTransaction({
        ...
        value: amount_in_wei,
        gas,
        gasPrice: w3.utils.toWei((w3.eth.gasPrice + 2).toString(), "gwei"),
        nonce: await w3.eth.getTransactionCount(w3.utils.toChecksumAddress(this.wallet_addr)),
        chainId: 1
      });

      const time = getEstimateConfirmTime(w3.utils.toWei((w3.eth.gasPrice + 2).toString(), "gwei"));
  ...
  async swapTokenToETH(amount: number): Promise<void> {
    ...
    try {
      ...
      const expectedAmountOut = await routerContract.methods.getAmountsOut(
        amountIn,
        [w3.utils.toChecksumAddress(this.token_addr), w3.utils.toChecksumAddress(WETH_ADDRESS)]
      ).call();

      const tokenAbi = get_erc20_abi();
      const tokenContract = new w3.eth.Contract(tokenAbi, w3.utils.toChecksumAddress(this.token_addr));
      ...
      if (allowance < amountIn) {
        ...
        const approveFunction = tokenContract.methods.approve(UNISWAP_ROUTER_V2, approveAmount * (10 ** decimal));
        ...
        const signedApproveTx = await w3.eth.accounts.signTransaction(approveTx, this.owner);
        const data = {
          jsonrpc: "2.0",
          method: "eth_sendRawTransaction",
      ...
      const swapFunction = routerContract.methods.swapExactTokensForETHSupportingFeeOnTransferTokens(
        amountIn,
        0,
        [w3.utils.toChecksumAddress(this.token_addr), w3.utils.toChecksumAddress(WETH_ADDRESS)],
        toAddress,
  ...
  async getBalance(): Promise<Balance> {
    const balanceWei = await w3.eth.getBalance(w3.utils.toChecksumAddress(this.wallet_addr));

    const abi = get_erc20_abi();
    const tokenContract = new w3.eth.Contract(abi, w3.utils.toChecksumAddress(this.token_addr));

    const balance = await tokenContract.methods.balanceOf(w3.utils.toChecksumAddress(this.wallet_addr)).call();
    const decimal = await tokenContract.methods.decimals().call();
  ...
  async waitForUpdateBalance(balanceBefore: Balance): Promise<void> {
    ...
    while (balanceAfter.token === balanceBefore.token && balanceAfter.eth === balanceBefore.eth) {
